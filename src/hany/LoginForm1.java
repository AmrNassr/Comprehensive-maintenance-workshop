...
private ImageIcon icon;
    public LoginForm1() {
        ...
        users();
        AutoCompleteDecorator.decorate(txtname);
    }
    ...
    private void initComponents() {
        ...
        txtname.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "" }));
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Image/IMG-20210624-WA0007.jpg"))); // NOI18N
...
public void login() throws IOException, ParseException {
        ...
        String q = "SELECT * FROM karem.users where username=? and password=?";
        try {
            p = DBConnect.appcon.prepareStatement(q);
            p.setString(1, name);
            p.setString(2, pass);

            ResultSet st = p.executeQuery();
            if (st.next()) {
                String sqll = "SELECT * FROM karem.users where username = '" + txtname.getSelectedItem()+ "'";
                pst = (PreparedStatement) DBConnect.appcon.prepareStatement(sqll);
                ResultSet rs = pst.executeQuery();
                if (rs.next()) {
                    nameee = rs.getString("username");
                    typee = rs.getString("type");
                    oldnew = rs.getString("oldnew");
                    section = rs.getString("section");
                }
                LL = name;
                new MainPage1(typee, nameee, oldnew, section).setVisible(true);
                this.dispose();

            } else {
                JOptionPane.showMessageDialog(null, "اسم المستخدم او كلمة السر غير صحيحة");
